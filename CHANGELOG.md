...
- c04220c: Upgrade Effect and @effect/vitest from 4.0.0-rc.115 to 4.0.0-rc.117 and require Effect ^4.0.0-rc.117 as a peer dependency.

## 0.4.1

### Patch Changes

- 1167b77: Upgrade to Effect 4.0.0-rc.115 and require Effect ^4.0.0-rc.115 as a peer dependency.

  Migrate schema transformations and Rational/grid arbitrary annotations to the RC APIs while preserving the existing wire formats.
...
- c45befb: Add dimensionless quantities: `Dimensionless` and `DimensionlessExact`, built on a new `Unitless` base unit, with paired constructors and extractors for `fraction`, `percent`, `perMille`, `basisPoints`, `partsPerMillion`, and `partsPerBillion`, plus `zero`, `one`, and `complement`.
